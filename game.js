...
function findMatch(playerId) {
  return new Promise((resolve) => {
    db.getMatches().then((matchesArray) => {
      ...
      for (let i = 0; i < matchCount; i += 1) {
        ...
        if (!gameOver && !playerIds.includes(playerId)) {
          if (playerIds.length === 0) {
            ...
            db.updateMatch(matchId, match).then(() => {
              resolve(match);
            });
            return;
          }
      ...
      db.updateMatch(match.matchId, match).then(() => {
        resolve(match);
      });
    });
  });
...
function play(io, playerId) {
  return new Promise((resolve) => {
    ...
    findMatch(playerId).then((match) => {
      if (match == null) {
        resolve(null);
      } else {
        const { matchId, board, players } = match;

        const { gameOver, winner } = isOver(board);
        const t = turn(board);
...
function addPiece(matchId, board, row, column, piece) {
  return new Promise((resolve) => {
    ...
    if (board[row][column] !== "") {
      ...
      db.updateBoard(matchId, b).then(() => {
        const { gameOver, winner } = isOver(b);
...
function submitMove(io, player, matchId, row, column, pit) {
  return new Promise((resolve) => {
    db.getMatch(matchId).then((match) => {
      const { board, players } = match;

      let { gameOver } = isOver(board);
      if (gameOver) {
        resolve(false);
        return;
      }

      addPiece(matchId, board, row, column, player).then((result) => {
        const { added } = result;
