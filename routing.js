...
module.exports = (app, io) => {
  ...
  app.post("/play", (req, res) => {
    console.log("BODY RESULT ==", req.body);
    ...
    if (missingArg != null) {
      ...
      game.play(io, playerId).then((result) => {
  ...
  app.post("/submitMove", (req, res) => {
    const { player, matchId, row, column, pit } = req.body;
    ...
    if (missingArg != null) {
      ...
      game.submitMove(io, player, matchId, row, column, pit).then((result) => {
        const resultMessage = JSON.stringify({ result });
        res.status(200).send(resultMessage);
      });
    }
